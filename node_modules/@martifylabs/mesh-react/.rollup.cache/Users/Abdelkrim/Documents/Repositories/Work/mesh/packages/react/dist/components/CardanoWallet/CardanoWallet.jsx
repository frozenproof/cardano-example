...
export const CardanoWallet = () => {
    ...
    return (<div onMouseEnter={() => setHideMenuList(false)} onMouseLeave={() => setHideMenuList(true)}>
      ...
      <StyledMenuList hidden={hideMenuList}>
        {!connected && walletList.length > 0 ? (walletList.map((wallet, index) => (<MenuItem key={index} icon={wallet.icon} name={wallet.name} connect={() => {
                connect(wallet.name);
                setHideMenuList(!hideMenuList);
            }} active={name === wallet.name}/>))) : walletList.length === 0 ? (<span>No Wallet Found.</span>) : (<>
            <ConnectedMenuItem label="Disconnect" onclick={() => {
                disconnect();
            }}/>
          </>)}
      </StyledMenuList>
